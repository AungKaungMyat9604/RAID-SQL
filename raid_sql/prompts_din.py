"""DIN-SQL few-shot prompt templates (legacy ablation via USE_DIN_STATIC_BANKS)."""
...
Q: "Find the title of course that is provided by both Statistics and Psychology departments."
Schema_links: [course.title,course.dept_name,Statistics,Psychology]
A: Let's think step by step. "Find the title of course that is provided by both Statistics and Psychology departments." can be solved by knowing the answer to the following sub-question "Find the titles of courses that is provided by Psychology departments".
The SQL query for the sub-question "Find the titles of courses that is provided by Psychology departments" is SELECT title FROM course WHERE dept_name  =  'Psychology'
So, the answer to the question "Find the title of course that is provided by both Statistics and Psychology departments." is =
Intermediate_representation: select course.title from course  where  course.dept_name = "Statistics" intersect select course.title from course where course.dept_name = "Psychology"
SQL: SELECT title FROM course WHERE dept_name  =  'Statistics' INTERSECT SELECT title FROM course WHERE dept_name  =  'Psychology'

Q: "Show the status shared by cities with population bigger than 1500 and smaller than 500."
Schema_links: [city.Status,city.Population,1500,500]
A: Let's think step by step. "Show the status shared by cities with population bigger than 1500 and smaller than 500." can be solved by knowing the answer to the following sub-question "What statuses do cities with population smaller than 500 have?".
The SQL query for the sub-question "What statuses do cities with population smaller than 500 have?" is SELECT Status FROM city WHERE Population  <  500
So, the answer to the question "Show the status shared by cities with population bigger than 1500 and smaller than 500." is =
Intermediate_representation: select city.Status from city where city.Population > 1500 intersect select city.Status from city where city.Population < 500
...
10) SELECT column order matters for official EX. Metric-first questions (How many / Count the number / what is the number / average / max / min) → AGG then key; else follow the question's attribute order with keys before trailing aggregates.
...
7) SELECT order: metric-first → AGG then key; else keys before aggregates in question order.
\n"""
